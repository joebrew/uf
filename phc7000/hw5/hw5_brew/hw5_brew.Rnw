...
\begin{center}
<<fig = TRUE>>=
aim <- runif(n = 100, min = 0, max = 100)
results <- runif(n = 100, min = 0, max = 100)
plot(aim, results, cex = ((1/abs(aim-results)+0.4)*2)^1.3,
     #cex = (abs(aim+results)/50) + (results/50),
...
plot(c(1,2),
     ...
     cex = 40,
     xlim = c(0,3),
     col = adjustcolor(c("grey", "black"), alpha.f = 0.3),
     xaxt = "n",
     yaxt = "n",
     xlab = NA,
     ylab = NA)
