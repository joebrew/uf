...
legend("topleft",
       ...
       lty = c(1,6),
       legend = c("Boys", "Girls"))
title(#main = "Obesity among American children", cex.main = 1.2,
      sub = "(Original chart, data from NIH)", cex.sub = 0.8)
@
\end{center}
...
my_colors <- c("red", gray.colors(67, start = 0.5, end = 0.9))

map("county", "fl", fill = TRUE, col = my_colors, border = "black")

legend("bottom",
       pch = NA,
       bty = "n",
       legend = "(Original map, produced in R)",
       cex = 0.8)
@
\end{center}
%----------------------------------------------------------------------------------------
