...
########### R CODE #################
library(xtable)
prob4 <- as.data.frame(as.character(c(1,2,3)))
colnames(prob4) <- "Model"
prob4$R.squared <- c(0.93396, 0.957128, 0.788385)
@
...
prob5$size <- ifelse(prob5$chest < 30,
                     ...
                     ifelse(prob5$chest >=30  &
                            ...
                            ifelse(prob5$chest >=40,
                                   "large",
                                   NA)))
prob5$model3 <- c(mod3(medium=TRUE),
                  mod3(small=TRUE),
                  mod3(small=TRUE),
                  mod3(large=TRUE),
                  mod3(large=TRUE))
...
<<results=tex, echo=FALSE>>=
print(xtable(prob5), digits=2, size="\\small")
@
...
<<echo=FALSE>>=
probs <- c(exp(-0.9256) / (1+exp(-0.9256)),
           exp(0.0907) / (1+exp(0.0907)), 
           exp(0.2049) / (1+exp(0.2049)))
@
