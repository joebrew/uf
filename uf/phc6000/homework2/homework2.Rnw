...
<<eval=TRUE, echo=TRUE, print=FALSE>>=
...
There is indeed an increased prevalence of disability in rural areas (\Sexpr{round(100*prevRural, digits=2)}\%) compared to urban areas (\Sexpr{round(100*prevUrban, digits=2)}\%).
...
\noindent Compared to those on a placebo, study participants taking aspirin had a relative risk (RR) of \Sexpr{round(relativeRisk, digits=4)} for Myocardial Infarction (MI).  This means that, at a 95\% confidence level, those taking aspirin had a \Sexpr{round(100*(1-relativeRisk), digits=2)}\% lower risk of MI compared to those on a placebo (95\% confidence interval of \Sexpr{round(ciLow, digits=2)}, \Sexpr{round(ciHigh, digits=2)}).  \\

\noindent In laymen's terms, we are 95\% confident that the true reduction in risk is between \Sexpr{round(ciLow, digits=2)}\% and \Sexpr{round(ciHigh, digits=2)}\%. 
...
\noindent Among those with pre-term birth (cases), the odds of being of lower socioeconomic status (relative to controls, those of upper SES) is \Sexpr{round(oddsRatio, digits=2)}.  The 95\% confidence interval is \Sexpr{round(ciLow, digits=2)}, \Sexpr{round(ciHigh, digits=2)}.  In other words, the risk of pre-term birth is 2.5 to 4 times higher for mothers of low SES.  
...
\noindent This study suggests increased risk of esophageal cancer with increased alcohol consumption.  Study participants with esophageal cancer were \Sexpr{round(oddsRatioLow, digits=2)} times more likely to consume 1-50 grams of alcohol per day (95\% confidence interval of \Sexpr{round(ciLowLow, digits=2)}, \Sexpr{round(ciLowHigh, digits=2)}) than those without cancer.  \\

\noindent At greater levels of consumption, the risk increases.  Those with esophageal cancer were \Sexpr{round(oddsRatioHigh, digits=2)} time more likely to consume 50-80 grams of alcohol per day (95\% confidence interval of of \Sexpr{round(ciHighLow, digits=2)}, \Sexpr{round(ciHighHigh, digits=2)}) than those without cancer. \\

\noindent A graphical display of this information is below:

\begin{center}
<<eval=TRUE, echo=FALSE, print=FALSE, fig=TRUE, height=3>>=
library(Hmisc)
par(mar=c(4,4,1,0))
par(oma=c(0,0,0,0))
mybp <- barplot(c(0, oddsRatioLow, oddsRatioHigh), ylim=c(-2, 9), names.arg=c("0", "1-50", "50-80"),
        xlab="Daily alcohol consumption (in grams)", ylab="Odds ratio", main="Esophageal cancer and alcohol consumption", space=0.2, col=adjustcolor("darkgreen", alpha.f=0.4), border=FALSE)
abline(h=0)
abline(h=-3:10, col=adjustcolor("black", alpha.f=0.2))
errbar(x=mybp[2,], y=oddsRatioLow, yplus=ciLowHigh, yminus=ciLowLow,
       add=TRUE, xlab="", pch=".", errbar.col=adjustcolor("blue", alpha.f=0.5), lwd=3)
errbar(x=mybp[3,], y=oddsRatioHigh, yplus=ciHighHigh, yminus=ciHighLow,
       add=TRUE, xlab="", pch=".", errbar.col=adjustcolor("blue", alpha.f=0.5), lwd=3)
@
\end{center}
\newpage
